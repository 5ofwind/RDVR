...
class InvNN(nn.Module):
	def __init__(self, channel_in=3, channel_out=3, subnet_constructor=None, block_num=[], down_num=1):
...
class PredictiveModuleMIMO2(nn.Module):
        def __init__(self, channel_in, block_num_rbm=8):
                ...
                residual_block = []
                for i in range(block_num_rbm):
                        residual_block.append(ResidualBlockNoBN(number1*2))
                self.residual_block = nn.Sequential(*residual_block)
...
class Net(nn.Module):
	def __init__(self, opt, subnet_constructor=None, down_num=1):
	...
	def forward(self, x, rev=False, hs=[], direction='f'):
		...
		if self.model == 'LSTM-VRN':
			...
			if not rev:
				...
				return out_x, out_z, out_x0 #out_x_final, or out_x, or out_x_final2
