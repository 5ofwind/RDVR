	...
	def __getitem__(self, index):
		...
		for v in self.LQ_frames_list:
			if self.data_type == 'mc':
				...
				#print(v, img_GT.shape)               
				#img_GT2=torch.tensor(img_GT).permute(2,0,1).unsqueeze(0)           
				#print(v, img_GT2.shape)                 
				#img_LQ = F.interpolate(img_GT2, scale_factor=0.25, mode="bicubic") #0.25
				...
				img_GT2=torch.tensor(img_GT).permute(2,0,1).unsqueeze(1)        
				#print(img_GT2.shape)
                
				img_LQ=Guassian_downsample(img_GT2)
				img_LQ=img_LQ.squeeze(1).permute(1,2,0).numpy()
				#img_LQ=img_GT################################################################
		...
		if self.opt['phase'] == 'train':
			...
			if self.LR_input:
				...
				#print(len(img_LQ_l))
				for j in range(len(img_GT_l)):
                    
					
					ttt=torch.tensor(img_GT_l[j]).permute(2,0,1).unsqueeze(1) 
					#ttt=ttt[[2, 1, 0],:, :, :]
                    
					#print(ttt.shape)                    
					ttt=Guassian_downsample(ttt,scale=2)
					#ttt=ttt[[2, 1, 0],:, :, :]      
                    
					#print(ttt.shape)                          
					img_LQ_l[j]=ttt.squeeze(1).permute(1,2,0).numpy()
                    
					#img_LQ_l[j]=util.imresize_np(img_GT_l[j], 1 / scale, True)
                    
					#print(img_LQ_l[j].shape)                    
		...
		if random.random() < 0.5:
			img_GTs=img_GTs[::-1, :, :, :]######################## temporal flip
			img_LQs=img_LQs[::-1, :, :, :]######################## temporal flip
