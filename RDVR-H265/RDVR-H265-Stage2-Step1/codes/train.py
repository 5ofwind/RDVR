...
from data.util import bgr2ycbcr, ycbcr2rgb
...
def cal_pnsr(opt, sr_img, gt_img, lr_img, lrgt_img):
    ...
    sr_img = bgr2ycbcr(sr_img, only_y=True)###
    gt_img = bgr2ycbcr(gt_img, only_y=True)###
    ...
    lrgt_img = bgr2ycbcr(lrgt_img, only_y=True)###
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        for _, train_data in enumerate(train_loader):
            ...
            if current_step % opt['train']['val_freq'] == 0 and rank <= 0:
                ...
                # log
                logger.info('# Validation # PSNR: {:.4e}, LR PSNR: {:.3e}'.format(avg_psnr, avg_psnr_lr))
                logger_val = logging.getLogger('val')  # validation logger
                logger_val.info('<epoch:{:3d}, iter:{:8,d}> psnr: {:.3e}, LR psnr: {:.3e}'.format(
