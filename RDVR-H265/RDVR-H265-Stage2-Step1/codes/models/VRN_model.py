...
class VRNModel(BaseModel):
    def __init__(self, opt):
        ...
        if opt["train"]:
            if opt["train"]["h265_sug"]:
                self.Quantization_H265_Suggrogate = Quantization_H265_Suggrogate(opt["train"]['lambda_corr'],opt["train"]["h265_q_train"],opt["gop"],opt["scale"])#train_len
            else:
                self.Quantization_H265 = Quantization_H265(opt["train"]["h265_q_train"],opt["gop"],opt["scale"])
                
        self.Quantization_H265_Stream = Quantization_H265_Stream(opt["network_G"]["h265_q_test_or_val"],opt["network_G"]["h265_keyint"],opt["network_G"]["scale"],opt)
        
        if opt['dist']:
            
            self.netG = DistributedDataParallel(self.netG, device_ids=[torch.cuda.current_device()])
            
            self.Quantization_H265_Suggrogate = DistributedDataParallel(self.Quantization_H265_Suggrogate, device_ids=[torch.cuda.current_device()])
            #self.Quantization_H265_Stream = DistributedDataParallel(self.Quantization_H265_Stream, device_ids=[torch.cuda.current_device()])
            
        else:
            self.netG = DataParallel(self.netG)
            #self.Quantization_H265_Suggrogate = DataParallel(self.Quantization_H265_Suggrogate)
            #self.Quantization_H265_Stream = DataParallel(self.Quantization_H265_Stream)
    ...
    def noise_video_suggrate(self,LR):
        ...
        b,c,h,w = LR.size()
        LR_flatten = LR.reshape(b*c//3,3,h,w)
        
        zero = torch.Tensor([0]).cuda(LR.device)
        mimick_loss = zero
        
        GlobalVar.set_Temporal_LEN(5)
        
        LR_flatten_quantized = self.Quantization(LR_flatten)
        if self.train_opt["noise_type"] == 'h265':
            if self.train_opt["h265_sug"]:
                LR_F,mimick_loss  = self.Quantization_H265_Suggrogate(LR_flatten_quantized)
            else:
                LR_F,mimick_loss  = self.Quantization_H265(LR_flatten_quantized)
                
            img_bpp = zero
        
        return LR_flatten,LR_F,zero,zero,mimick_loss,img_bpp
    ...
    def loss_modified_y(self, out, y):
        if self.opt['model'] == 'LSTM-VRN':
            l_forw_fit = self.train_opt['lambda_modified_y'] * self.Reconstruction_forw(out, y)
            return l_forw_fit
        elif self.opt['model'] == 'MIMO-VRN':
            l_forw_fit = 0
            for i in range(out.shape[1]):
                l_forw_fit += self.train_opt['lambda_modified_y'] * self.Reconstruction_forw(out[:, i], y[:, i])
            return l_forw_fit        
    ...
    def optimize_parameters(self):
        ...
        if self.opt['model'] == 'LSTM-VRN':
            ...
            self.output=LR_ref3.reshape(-1, self.gop*3, h // 2, w // 2)
            
            y = self.Quantization(self.output[:, :3 * self.gop, :, :])
            
            y,LR_codec_recons,distortion_loss,distribution_loss,mimick_loss,img_bpp = self.noise_video_suggrate(y)
            mimick_loss = mimick_loss.mean()*self.train_opt['lambda_mimick_loss']
            
            LR_codec_recons=LR_codec_recons.reshape(b, -1, h//2, w//2)
            out_x, out_z, out_modified_y = self.netG(x=[LR_codec_recons, None], rev=True)
            
            LR_codec_recons = LR_codec_recons.reshape(-1, self.gop, 3, h // 2, w // 2)
            l_modified_y = self.loss_modified_y(LR_codec_recons, LR_ref)
            
            l_back_rec = self.loss_back_rec(out_x.reshape(-1, self.gop, 3, h, w), self.input)
            # total loss
            loss = l_forw_fit + l_back_rec + mimick_loss + l_modified_y
        ...
        #self.log_dict['l_modified_y'] = l_modified_y.item()
    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            if self.opt['model'] == 'LSTM-VRN':
                ...
                ppppp=torch.randn(cc*5, 3,self.input_LR[0].shape[4],self.input_LR[0].shape[3])
                self.Quantization_H265_Stream.open_writer(ppppp.cpu().device,self.input_LR[0].shape[4],self.input_LR[0].shape[3])####w//2,h//2 
                
                for i in range(cc):
                    t00 = time.time()
                    self.output = self.netG(x=self.input[0].reshape(b, -1, h, w))
                    t11 = time.time()
                    total_down_time=(t11-t00)/self.gop
                    
                    print("===> DownTimer: %.4f sec\n\t." % total_down_time)
                    
                    outout=self.output[:,0:15,:,:]
                    
                    ww=torch.tensor(outout.shape[3]).numpy()
                    hh=torch.tensor(outout.shape[2]).numpy()
                    
                    self.Quantization_H265_Stream.write_multi_frames(outout.cuda(0).detach().to("cpu"))
                ...
                _,img_distri = self.Quantization_H265_Stream.close_writer()
                print('bpp',img_distri)
                self.img_distri_total=self.img_distri_total+img_distri
                
                logger.info('bpp: {:.6f}.'.format(img_distri))
                ...
                outsouts2 = []
                for seg_i in range(cc):
                    v_seg = self.Quantization_H265_Stream.read_multi_frames(self.gop)
                    outsouts2+=[v_seg]
                outout2 = torch.stack(outsouts2,dim=0)#cat
                ...
                for i in range(cc):
                    ...
                    ppp=outout2[i].reshape(b, -1, h//2, w//2)
                    
                    for j in range(self.gop):
                        forw_L[i*self.gop+j]=ppp[:,j*3:j*3+3,:,:]
    ...
    def get_current_visuals(self):
        ...
        out_dict['bpp_total']=self.img_distri_total
