...
class VRNModel(BaseModel):
    ...
    def optimize_parameters(self):
        ...
        if self.opt['model'] == 'LSTM-VRN':
            ...
            out_x, out_z, out_lrs_modify_LR = self.netG(x=[y, None], rev=True) 

            out_lrs_modify_LR = out_lrs_modify_LR[:, :3 * self.gop, :, :].reshape(-1, self.gop, 3, h // 4, w // 4)
            
            l_forw_fit2 = self.loss_forward(out_lrs_modify_LR, LR_ref)
        ...
        with torch.no_grad():
            ...
            if self.opt['model'] == 'LSTM-VRN':
                ...
                for i in range(cc):
                    ...
                    ppp = ppp.reshape(b, -1, h//4, w//4)
                    
                    y = self.Quantization(ppp)#self.output[:, :3 * self.gop, :, :]#.contiguous()#Variable(X, volatile=True)


                    
                    out_x, out_z, out_lrs_modify_LR = self.netG(x=[y, None], rev=True)#out_x0, 
                    ...
                    for j in range(self.gop):    
                        forw_L2.append(out_lrs_modify_LR[:, j])
            ...
            self.forw_L = torch.stack(forw_L2, dim=1)
