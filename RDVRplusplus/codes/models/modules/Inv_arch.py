...
class ResidualBlockNoBN(nn.Module):
	def __init__(self, nf=64, model='MIMO-VRN'):
		...
		# initialization
		initialize_weights_xavier([self.conv1, self.conv2], 0.1)#0.1#1 is right
...
class PredictiveModuleMIMO(nn.Module):
        def __init__(self, channel_in, nf, block_num_rbm=13):
...
class PredictiveModuleMIMO2(nn.Module):
        def __init__(self, channel_in, block_num_rbm=11):
                super(PredictiveModuleMIMO2, self).__init__()

                self.lrelu = nn.LeakyReLU(negative_slope=0.2, inplace=True)

                number1=33
                self.conv_in1 = nn.Conv2d(3, number1, 3, 1, 1, bias=True)
                self.conv_in2 = nn.Conv2d(3, number1, 3, 1, 1, bias=True)

                self.conv_first_11 = nn.Conv2d(number1, number1, 3, 2, 1, bias=True)
                self.conv_first_12 = nn.Conv2d(number1, number1, 3, 2, 1, bias=True)

                residual_block = []
                for i in range(block_num_rbm):
                        residual_block.append(ResidualBlockNoBN(number1*2))#*3#channel_in #192
                self.residual_block = nn.Sequential(*residual_block)


                self.upconv1 = nn.Conv2d(number1*2, number1 * 4, 3, 1, 1, bias=True)
                self.pixel_shuffle = nn.PixelShuffle(2)
                self.HRconv = nn.Conv2d(number1, number1, 3, 1, 1, bias=True)

                self.conv_last = nn.Conv2d(number1, 3, 3, 1, 1, bias=True)

                for p in self.parameters():
                    p.requires_grad=False

        def forward(self, x):
                x1 = self.lrelu(self.conv_in1(x[:,0:3,:,:]))
                x2 = self.lrelu(self.conv_in2(x[:,3:6,:,:]))
                ...
                x = self.lrelu(self.HRconv(x))#self.lrelu
                x = self.conv_last(x)

                return x

class PredictiveModuleMIMO_modify_LR(nn.Module):
        def __init__(self, channel_in, nf, block_num_rbm=7):
                super(PredictiveModuleMIMO_modify_LR, self).__init__()
                nf=50
                self.conv_in = nn.Conv2d(channel_in, nf, 3, 1, 1, bias=True)
                residual_block = []
                for i in range(block_num_rbm):
                        residual_block.append(ResidualBlockNoBN(nf))
                self.residual_block = nn.Sequential(*residual_block)
                self.conv_out = nn.Conv2d(nf, channel_in, 3, 1, 1, bias=True)

        def forward(self, x):
                x = self.conv_in(x)
                return self.conv_out(self.residual_block(x))
	...
	def __init__(self, opt, subnet_constructor=None, down_num=2):
		...
		if self.model == 'LSTM-VRN':
			...
			initialize_weights_xavier(self.z_comb)
                        ...
                        self.pm_modify_LR = PredictiveModuleMIMO_modify_LR(self.channel_in, self.nf)
                        self.pm2 = PredictiveModuleMIMO2(self.channel_in)
		else:
			raise Exception('Model should be either LSTM-VRN or MIMO-VRN.')
	...
	def forward(self, x, rev=False, hs=[], direction='f'):
		...
		if self.model == 'LSTM-VRN':
			...
			if not rev:
				...
				out_y_modify_LR=self.pm_modify_LR(y[:, :15, :, :])+y[:, :15, :, :]
				...
				out_x=out_x.clone()
				out_x2=(self.pm2(out_x[:,3:9,:,:])+out_x[:,3:6,:,:]).clone()
				out_x[:,3:6,:,:]=out_x2.clone()
				out_x3=torch.cat([out_x[:,9:12,:,:],out_x[:,6:9,:,:]],1)
				out_x3=(self.pm2(out_x3)+out_x[:,9:12,:,:]).clone()
				out_x[:,9:12,:,:]=out_x3.clone()
				out_x=out_x.clone()
				                
				out_x4=(self.pm2(out_x[:,0:6,:,:])+out_x[:,0:3,:,:]).clone()
				out_x5=torch.cat([out_x[:,12:15,:,:],out_x[:,9:12,:,:]],1)
				out_x5=(self.pm2(out_x5)+out_x[:,12:15,:,:]).clone()
				out_x[:,0:3,:,:]=out_x4.clone()
				out_x[:,12:15,:,:]=out_x5.clone()
				                
				return out_x, out_z, out_y_modify_LR
