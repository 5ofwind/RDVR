...
def cal_pnsr_ssim(sr_img, gt_img, lr_img, lrgt_img):
    ...
    #################################3util.save_img(lr_img, save_img_path)
    ...
    if gt_img.shape[2] == 3:  # RGB image
        ...
        writer.writerow([osp.join(folder, img_name), psnr_y, psnr_y_lr, ssim_y, ssim_y_lr])
        logger.info(
            '{:20s} - PSNR_Y: {:.6f} dB; SSIM_Y: {:.6f}. PSNR_Y: {:.6f} dB; SSIM_Y: {:.6f}.'.
                format(osp.join(folder, img_name), psnr_y, ssim_y, psnr_y_lr, ssim_y_lr))
...
for test_loader in test_loaders:
    ...
    with open(osp.join(opt['path']['log'], 'test_' + opt['name'] + '_test.csv'), 'w') as f:
        ...
        for data in test_loader:
            #print(data)
            data['GT_path']=data['GT_path'][0:7]#[0:5]#[1:6]
            print(data['GT_path'][0])
            #print(data['GT_path'])
            ###if data['GT_path'][0][-5] != '1' and data['GT_path'][0][-5]!='7':
            ...
            if test_set_name == 'Vid4':
                ...
                folder2 = osp.split(osp.dirname(data['GT_path'][0][0]))[1]
                folder3 = osp.split(osp.split(osp.dirname(data['GT_path'][0][0]))[0])[1]
                folder = osp.join(folder3,folder2)
                print(folder)
